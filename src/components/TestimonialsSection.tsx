...
const TestimonialsSection = () => {
  ...
  const {
    ref: testimonialRef,
    isIntersecting: testimonialVisible
  } = useIntersectionObserver();
  const {
    ref: ctaRef,
  ...
  return <section className="section-padding bg-gradient-to-br from-background to-primary/5">
      <div className="container-wide">
        ...
        <div ref={statsRef} className="grid md:grid-cols-3 gap-6 mb-12">
          ...
          return <Card key={index} className={`text-center p-6 ${colorClasses[stat.color]} border-2 transition-all duration-700 ${statsVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95'}`} style={{
        ...
        {/* Main Testimonial */}
        <Card ref={testimonialRef} className={`card-gradient mb-8 transition-all duration-700 delay-600 ${testimonialVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95'}`}>
                  ...
                  <div className="mt-6 p-4 bg-gradient-card rounded-xl border border-primary/10">
                    ...
                    <div className="flex items-center gap-2 flex-nowrap">
                      ...
                      <Badge className="bg-success text-success-foreground whitespace-nowrap">
                        {currentTestimonial.salaryIncrease !== 'First Job' ? `${currentTestimonial.salaryIncrease} Salary Increase` : 'First Job Success'}
                      </Badge>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex justify-center items-center gap-4">
          <Button variant="outline" size="sm" onClick={prevSlide} className="btn-outline">
            <ChevronLeft className="h-4 w-4" />
          </Button>

          <div className="flex gap-2">
            {testimonials.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-primary' : 'bg-muted'}`} />)}
          </div>

          <Button variant="outline" size="sm" onClick={nextSlide} className="btn-outline">
        ...
        <div className="text-center mt-12">
          <Card ref={ctaRef} className={`card-gradient inline-block p-6 max-w-2xl transition-all duration-700 delay-900 ${ctaVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95'}`}>
